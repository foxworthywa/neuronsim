...
`#2d8fd5` at 40 % opacity; T-tubule `#8a7a50`; troponin/tropomyosin cover `#6b7280`.

| view | opts | extra members | what it shows |
|---|---|---|---|
| `zoomMuscle` | `{}` | `next()`, `index`, `last` | five slides: person lifting a cup → arm → muscle (fascicles) → one fascicle → one fibre with a small trace blip + force gauge bump on the last slide. Same shape as the neuron `zoom` view. |
| `fibre` | `{ caption, labels, stimulator, showForce }` | `onPart = fn(part)`, `highlight(part)`, `parts()` | whole fibre, horizontal, ≈ 3 cm drawn long; sarcolemma coloured per segment by `kit.vColor(c.V)`; T-tubule openings drawn along it, SR sleeve, myofibril stripes; the motor end plate in the middle (a small nerve ending stub; no nerve beyond it); parts clickable with `data-part` ∈ `sarcolemma, ttubule, sr, myofibril, endplate`; the recording electrode (`kit.app.electrode`) at `profile.comp`… actually at the compartment named by `opts.electrodeAt` (default `'R3'`); a stimulating electrode pair at the end plate when `opts.stimulator`; the whole fibre shortens visibly with `sim.force` (up to ≈ 10 %) and a **force gauge** (vertical bar, 0–1, labelled) when `opts.showForce`. `narrate()` describes the state (rest, impulse spreading, Ca²⁺ up, force rising/falling, tetanus, rigor). |
